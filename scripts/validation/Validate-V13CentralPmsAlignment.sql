...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON t.oid = e.enumtypid JOIN pg_namespace n ON n.oid = t.typnamespace WHERE n.nspname = 'identity' AND t.typname = 'human_session_audience_enum' AND e.enumlabel = 'NATIVE_PARKING_APP') THEN missing := array_append(missing, 'identity.human_session_audience_enum.NATIVE_PARKING_APP'); END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'identity' AND table_name = 'local_credentials' AND column_name = 'temporary_password_expires_at' AND data_type = 'timestamp with time zone') THEN missing := array_append(missing, 'identity.local_credentials.temporary_password_expires_at timestamptz'); END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint con JOIN pg_class cls ON cls.oid = con.conrelid JOIN pg_namespace n ON n.oid = cls.relnamespace WHERE n.nspname = 'identity' AND cls.relname = 'local_credentials' AND con.conname = 'ck_local_credentials_temporary_password_expiry' AND con.contype = 'c') THEN missing := array_append(missing, 'ck_local_credentials_temporary_password_expiry'); END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'identity' AND tablename = 'local_credentials' AND indexname = 'ix_local_credentials_temporary_password_expiry') THEN missing := array_append(missing, 'ix_local_credentials_temporary_password_expiry'); END IF;
    ...
    IF to_regclass('sessions.vendor_session_projections') IS NULL THEN missing := array_append(missing, 'sessions.vendor_session_projections'); END IF;
    IF to_regclass('sessions.vendor_session_projection_sync_targets') IS NULL THEN missing := array_append(missing, 'sessions.vendor_session_projection_sync_targets'); END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'sessions' AND table_name = 'vendor_session_projections' AND column_name = 'source_adapter_identity_id' AND data_type = 'uuid') THEN missing := array_append(missing, 'sessions.vendor_session_projections.source_adapter_identity_id'); END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'sessions' AND table_name = 'vendor_session_projection_sync_targets' AND column_name = 'last_lock_contention_at' AND data_type = 'timestamp with time zone') THEN missing := array_append(missing, 'sessions.vendor_session_projection_sync_targets.last_lock_contention_at'); END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'sessions' AND table_name = 'vendor_session_projection_sync_targets' AND column_name = 'lock_contention_count' AND data_type = 'integer') THEN missing := array_append(missing, 'sessions.vendor_session_projection_sync_targets.lock_contention_count'); END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint con JOIN pg_class cls ON cls.oid = con.conrelid JOIN pg_namespace n ON n.oid = cls.relnamespace WHERE n.nspname = 'sessions' AND cls.relname = 'vendor_session_projections' AND con.conname = 'uq_vendor_session_projections__target_stable_identity' AND con.contype = 'u') THEN missing := array_append(missing, 'uq_vendor_session_projections__target_stable_identity'); END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_constraint con JOIN pg_class cls ON cls.oid = con.conrelid JOIN pg_namespace n ON n.oid = cls.relnamespace WHERE n.nspname = 'sessions' AND cls.relname = 'vendor_session_projection_sync_targets' AND con.conname = 'ck_vendor_session_projection_sync_targets__health_status' AND con.contype = 'c' AND pg_get_constraintdef(con.oid) LIKE '%DEFERRED%') THEN missing := array_append(missing, 'projection sync target DEFERRED health constraint'); END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'sessions' AND indexname = 'ux_vendor_session_projections__target_vendor_record_guid') THEN missing := array_append(missing, 'ux_vendor_session_projections__target_vendor_record_guid'); END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE schemaname = 'sessions' AND indexname = 'ux_vendor_session_projection_sync_targets__scope') THEN missing := array_append(missing, 'ux_vendor_session_projection_sync_targets__scope'); END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'core' AND table_name = 'parking_sessions' AND column_name = 'source_adapter_identity_id' AND data_type = 'uuid') THEN missing := array_append(missing, 'core.parking_sessions.source_adapter_identity_id'); END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'core' AND table_name = 'tariff_snapshots' AND column_name = 'source_adapter_identity_id' AND data_type = 'uuid') THEN missing := array_append(missing, 'core.tariff_snapshots.source_adapter_identity_id'); END IF;
