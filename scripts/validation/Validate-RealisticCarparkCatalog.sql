-- Validate the exact realistic carpark catalog and its non-operational posture.
BEGIN;
...
BEGIN
  ...
  IF (SELECT count(*) FROM ep_realistic_catalog_sites WHERE site_type = 'OPEN_LOT') <> 5
     ...
     OR (SELECT count(*) FROM ep_realistic_catalog_sites WHERE site_type = 'MALL_PARKING') <> 9
     OR (SELECT count(*) FROM ep_realistic_catalog_sites WHERE site_type = 'MIXED_USE_PROPERTY') <> 1
     OR (SELECT count(*) FROM ep_realistic_catalog_sites WHERE site_type = 'OTHER') <> 29 THEN
    RAISE EXCEPTION 'Realistic carpark validation failed: Site type distribution changed.';
  END IF;

  IF (SELECT count(*) FROM ep_realistic_catalog_sites e JOIN ep_realistic_catalog_groups g USING(site_group_id) WHERE g.site_group_code = 'MACTAN-NEW-TOWN' AND e.jurisdiction_id = '23104fc9-a144-381c-4347-ccb2aa1a2998' AND e.psgc_code = '0731100000') <> 6
     OR (SELECT count(*) FROM ep_realistic_catalog_sites e JOIN ep_realistic_catalog_groups g USING(site_group_id) WHERE g.site_group_code = 'BRIDGETOWNE' AND e.jurisdiction_id = '20650612-8f91-3b4a-bba8-5d8afe29ef5a') <> 2 THEN
    RAISE EXCEPTION 'Realistic carpark validation failed: Mactan New Town or Bridgetowne reconciliation changed.';
  END IF;

  IF EXISTS (
    SELECT 1 FROM integration.adapter_mappings m
    WHERE m.site_group_id IN (SELECT site_group_id FROM ep_realistic_catalog_groups)
       OR m.site_id IN (SELECT site_id FROM ep_realistic_catalog_sites)
  ) OR EXISTS (
    SELECT 1 FROM discounts.statutory_discount_policy_versions p
    WHERE p.site_group_id IN (SELECT site_group_id FROM ep_realistic_catalog_groups)
       OR p.site_id IN (SELECT site_id FROM ep_realistic_catalog_sites)
  ) THEN
    RAISE EXCEPTION 'Realistic carpark validation failed: integration activation or executable statutory policy exists.';
  END IF;
END $$;

SELECT 'Realistic carpark canonical catalog validation passed.' AS validation_result;
ROLLBACK;
